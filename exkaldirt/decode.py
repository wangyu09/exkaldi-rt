...
from exkaldirt.base import Component, PIPE, Vector, Text
from exkaldirt.base import encode_vector
...
def softmax(data,axis=1):
	...
	Args:
		_data_: (2-d array) Probability.
		_axis_: (int) Which dim. to do softmax.
...
def log_softmax(data,axis=1):
	...
	Args:
		_data_: (2-d array) Probability.
		_axis_: (int) Which dim. to do log softmax.
...
def load_symbol_table(filePath):
	'''
	Load word to int ID table.

	Args:
		_filePath_: (str) words.txt file path.
...
def get_pdf_dim(hmmFile):
	'''
	Get PDF dim. from a HMM file.

	Args:
		_hmmFile_: (str) HMM file path. 
	'''
...
class AcousticEstimator(Component):
	'''
	Acoustic probability estimator.
	'''
	def __init__(self,featDim,batchSize=16,
										applySoftmax=False,applyLog=True,
										priors=None,leftContext=0,rightContext=0,name=None):
		'''
		Args:
			_featDim_: (int) Feature dim.
			_batchSize_: (int) Batch size.
			_applySoftmax_: (bool) If True, apply softmax to the probability.
			_applyLog_: (bool) If True, apply log to the probability.
			_priors_: (1-d array) The prior probability.
			_leftContext_: (int) The left context size. If > 0, we will pad zeros at the beginning.
			_rightContext_: (int) The right context size. If > 0, we will pad zeros at the tail.
			_name_: (str) Name.
		'''
		...
		# The work place (left and right context space are prepared for TDNN)
		...
		self.__left = leftContext
		self.__right = rightContext
		self.__cover = leftContext + rightContext
		self.__width = batchSize + leftContext + rightContext
		self.__featureBuffer = np.zeros([ self.__width + rightContext, featDim],dtype="float32") # Keep an extra rightContext space as the last padding
			...
			while True:
				...
				if self.__duration == 0:
					if self.__finalStep:
						...
						continue
				# A normal case
				if self.__tailIndex == self.__width:
					avaliableFeat = self.__featureBuffer[:self.__width]
				# Endpoint case
				else:
					avaliableFeat = self.__featureBuffer[:self.__tailIndex+self.__right] # Right padding
				# Compute if necessary
				if len(avaliableFeat) > 0:
					probs = self.acoustic_function(avaliableFeat)
					assert isinstance(probs,np.ndarray) and len(probs.shape) == 2, f"{self.name}: Acoustic function must return a 2-d array."
...
class WfstDecoder(Component):
	...
	def __init__(self,probDim,batchSize,symbolTable,silencePhones,frameShiftSec,tmodel,graph,
		...
		'''
		Args:
			_probDim_: (int) The dim. of probability.
			_batchSize_: (int) Batch size.
			_symbolTable_: (str) Word to id file.
			_silencePhones_: (str) Silence phone IDs concatenated with ":" .
			_frameShiftSec_: (float) Seconds of each frame shift.
			_tmodel_: (str) HMM model file path.
			_graph_: (str) HCLG file path.
			_wordBoundary_: (str) Word boundary file path.
			_nBests_: (int) N bests.
			_beam_: (float) Beam.
			_maxActive_: (int) Maximum active tokens.
			_minActive_: (int) Minimum active tokens.
			_latticeBeam_: (float) Lattice beam.
			_pruneInterval_: .
			_beamDelta_: .
			_hashRatio_: .
			_pruneScale_: .
