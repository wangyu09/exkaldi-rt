...
from A_NN import make_DNN_model
...
args.add("--testModelDir", abbr="-t", dtype=str, default="", discription="If not empty, do test step.")
args.add("--root", abbr="-r", dtype=str, default=rootdir, discription="The root dir of data.")
args.add("--feat", abbr="-f", dtype=str, default="mfcc", discription="Feature.")
args.add("--cmn", abbr="-c", dtype=bool, default=True, discription="CMVN.")
args.add("--delta", abbr="-d", dtype=int, default=2, discription="Delta feature.")
args.add("--splice", abbr="-s", dtype=int, default=10, discription="Splice feature.")
...
def prepare_data(training=True):
  ...
  pdfAli = ali.to_numpy(aliType="pdfID",hmm=f"{args.feat}/exp/tri3b_ali_{flag}/final.mdl")
  del ali
...
def train():
  ...
  _, devDataset = prepare_data(training=False)
  ...
  #------------------------ Train Step -----------------------------
  model = make_DNN_model(featDim,pdfDim)
  model.summary()
...
def compute_dev_wer():
  ...
  try:
    for ModelPathID in range(30,0,-1):
